...
var JSHandle = function(){
    ...
    var updateData = function () {
        ...
        $('table').delegate('.changeNewsBtn','click',function () {
            ...
            $.ajax({
                type:"GET",
                url:'selectOne?newsId='+newsID,
                error: function(request) {
                    console.log("操作失败");
                },
                success: function(msg) {
                    //解析从php返回的数据
                    var callback = JSON.parse(msg)[0];
                    //将数据显示在模态窗的表单内
    ...
    var inputText = function () {
        $('input').on('blur',inputTest(this));
        $('textarea').on('blur',inputTest(this));
        function inputTest(oField ){
